...
{
    fopen("/home/user/end/.pass", "r"); // FILE *fopen(const char *path, const char *mode);
    memset();                           // void *memset(void *s, int c, size_t n);
    fread();                            // size_t fread(void *ptr, size_t size, size_t nmembFILE *" stream );
...
//    0x08048531 <+61>: 	rep stos DWORD PTR es:[edi],eax         # => memset();
